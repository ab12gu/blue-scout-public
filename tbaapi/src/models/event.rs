...
pub struct Event {
    ...
    #[serde(rename = "key")]
    pub key: String,
    /// Official name of event on record either provided by FIRST or organizers
    /// of offseason event.
    #[serde(rename = "name")]
    pub name: String,
    ...
    /// Week of the event relative to the first official season event,
    /// zero-indexed. Only valid for Regionals, Districts, and District
    /// Championships. Null otherwise. (Eg. A season with a week 0 'preseason'
    /// event does not count, and week 1 events will show 0 here. Seasons with a
    /// week 0.5 regional event will show week 0 for those event(s) and week 1
    /// for week 1 events and so on.)
    ...
    pub lng: Option<f64>,
    /// Name of the location at the address for the event, eg. Blue Alliance
    /// High School.
    ...
    /// The FIRST internal Event ID, used to link to the event on the FRC
    /// webpage.
    #[serde(rename = "first_event_id", deserialize_with = "Option::deserialize")]
    pub first_event_id: Option<String>,
    /// Public facing event code used by FIRST (on frc-events.firstinspires.org,
    /// for example)
    ...
    /// The TBA Event key that represents the event's parent. Used to link back
    /// to the event from a division event. It is also the inverse relation of
    /// `divison_keys`.
...
impl Event {
    pub fn new(
        ...
        event_code: String,
        event_type: i32,
        district: Option<models::DistrictList>,
        city: Option<String>,
        ...
        webcasts: Vec<models::Webcast>,
        division_keys: Vec<String>,
        ...
        Event {
            ...
            event_code,
            event_type,
            district: if let Some(x) = district {
                Some(Box::new(x))
            } else {
                None
            },

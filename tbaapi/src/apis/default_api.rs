...
/// Gets a large blob of data that is used on the frontend for searching. May
/// change without notice.
pub async fn get_search_index(
    configuration: &configuration::Configuration,
) -> Result<models::SearchIndex, Error<GetSearchIndexError>> {
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to `models::SearchIndex`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
                    "Received `{unknown_type}` content type response that cannot be converted to `models::SearchIndex`"
                ))));
            }
        }
    } else {
